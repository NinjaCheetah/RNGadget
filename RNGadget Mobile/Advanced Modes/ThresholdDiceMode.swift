//
//  ThresholdDiceMode.swift
//  RNGTool Mobile
//
//  Created by Campbell on 8/18/25.
//

import SwiftUI
import CoreHaptics

struct ThresholdDiceMode: View {
    @EnvironmentObject var settingsData: SettingsData
    @Environment(\.accessibilityReduceMotion) var reduceMotion
    @State private var engine: CHHapticEngine?
    @State private var numDice: Int = 1
    @State private var numSides: Int = 4
    @State private var confirmReset: Bool = false
    @State private var randomNumbers: [Int] = [0]
    @State private var diceImages: [String] = Array(repeating: "d6-1", count: 12)
    ...
    func resetGen() {
        ...
        rollTask = nil
        numDice = 1
        diceImages = Array(repeating: "d6-1", count: 12)
        confirmReset = false
    }
    ...
    func roll() {
        ...
        for n in 0..<randomNumbers.count{
            if(numDice > n) { diceImages[n] = "d6-\(randomNumbers[n])" }
        }
    }
    ...
    var body: some View {
        GeometryReader { geometry in
            VStack(spacing: 0) {
                ...
                VStack(spacing: 10) {
                    Text("Number of Dice")
                    TextField("Enter a number", value: $numDice, format: .number)
                        .keyboardType(.numberPad)
                        .textFieldStyle(.roundedBorder)
                        .frame(maxWidth: .infinity)
                        .focused($fieldFocused, equals: .numDice)
                        .disabled(rollTask != nil)
                    Text("Threshold")
                    TextField("Enter a number", value: $numSides, format: .number)
                        .keyboardType(.numberPad)
                        .textFieldStyle(.roundedBorder)
                        .frame(maxWidth: .infinity)
                        .focused($fieldFocused, equals: .numSides)
                        .disabled(rollTask != nil)
                    Button(action:{
                        startRoll()
                    }) {
        ...
        .navigationTitle("Threshold Dice")
        ...
        .alert("Threshold Dice Mode", isPresented: $showingExplainer, actions: {}, message: {
            Text("WIP Explainer Text")
        })
    }
}

#Preview {
    ThresholdDiceMode().environmentObject(SettingsData())
}
